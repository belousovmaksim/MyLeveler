...
class MainActivity : AppCompatActivity() {
    ...
    private var mField = FloatArray(3)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val sensorMag = sManager.getDefaultSensor(Sensor.TYPE_MAGNETIC_FIELD)
        val sListener = object : SensorEventListener {
            override fun onSensorChanged(event: SensorEvent?) {

                when (event?.sensor?.type) {
                    Sensor.TYPE_ACCELEROMETER -> acceleration = event.values.clone()
                    Sensor.TYPE_MAGNETIC_FIELD -> mField = event.values.clone()
                }

                SensorManager.getRotationMatrix(gravity, magnetic, acceleration, mField)
        ...
        sManager.registerListener(sListener, sensorMag, SensorManager.SENSOR_DELAY_FASTEST)
